...
class Game2
  ...
  # ゲームシーンの初期化
  def initialize
    @image = Util.load_image("bg2.jpg")
    path = "#{File.dirname(__FILE__)}/../shimane_words_2.json"
    @shimane_word_manager = ShimaneWordManager.new path, 3, 1.5
  ...
  def clear
        ...
        @tama = Tama.new(Window.width/2,Window.height - 150,Image.load("#{File.dirname(__FILE__)}/../images/player_shimanekko.png"))
        @ima = Ima.new(-500,-500,Image.load("#{File.dirname(__FILE__)}/../images/aji.png"))
      	@score = 0
      	@timer = Timer.new 5
      	@shot_x = 0
        @shot_y = 0
  ...
  def play
    ...
    # 描画
    Window.drawFont(Window.width - 140,Window.height - 100,"score",@font)
    Window.drawFont(Window.width - 140,Window.height - 60,@score.to_s,@font)
  ...
  def score_judge
    ...
    if cleared?
	clear
      Director.change_scene(:stage2to3)
    elsif missed? 
	clear
